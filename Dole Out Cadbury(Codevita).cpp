...
if(mx==mn)
{dp[mx][mn]=1;return 1;}
dp[mx][mn] = 1+func(max(mn,mx-mn),min(mn,mx-mn));
return dp[mx][mn];
}
*/
int main()
{
int minl,maxl,minw,maxw,sum=0;
cin>>minl>>maxl>>minw>>maxw;
/*for(int i=minl;i<=maxl;i++)
for(int j=minw;j<=maxw;j++)
sum+=func(max(i,j),min(i,j));
*/
...
dp[i][j]=dp[j][i]=max(i,j);
else
dp[i][j]=dp[j][i]=1+dp[min(i,j)][max(i,j)-min(i,j)];
}
for(int i=minl;i<=maxl;i++)
for(int j=minw;j<=maxw;j++)
sum+=dp[i][j];
